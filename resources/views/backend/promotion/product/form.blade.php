...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                @if( empty($sRow) )
                <form action="{{ route('backend.promotion.product.store') }}" method="POST" autocomplete="off">
                @else
                <form action="{{ route('backend.promotion.product.update', $sRow->id ) }}" method="POST" autocomplete="off">
                ...
                {{ csrf_field() }}
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="product_id">
                                      @foreach($sProduct AS $r)
                                        <option value="{{$r->id}}" {{ (@$sRow->product_id==$r->id)?'selected':'' }}>{{$r->name.' '.$r->subname}}</option>
                                        @endforeach
                                    </select>
                                    @endif
                                </div>
                    ...
                    <div class="form-group mb-0 row">
                        <div class="col-md-6">
                            <a class="btn btn-secondary waves-effect" href="{{ route('backend.promotion.product.index') }}">
